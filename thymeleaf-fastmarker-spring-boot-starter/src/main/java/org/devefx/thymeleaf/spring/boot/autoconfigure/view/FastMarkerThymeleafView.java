...
public class FastMarkerThymeleafView extends AbstractThymeleafView {
    ...
    protected void renderFragment(final IFragmentSpec fragmentSpecToRender, final Map<String, ?> model, final HttpServletRequest request, 
        ...
        final SpringWebAndPreFragmentHandlerContext context =
                new SpringWebAndPreFragmentHandlerContext(request, response, servletContext, getLocale(), mergedModel, getApplicationContext());
    ...
    class SpringWebAndPreFragmentHandlerContext extends SpringWebContext implements PreFragmentHandlerContent {
        
        public SpringWebAndPreFragmentHandlerContext(HttpServletRequest request, HttpServletResponse response,
                ServletContext servletContext, Locale locale,
                Map<String, ?> variables, ApplicationContext appctx) {
            super(request, response, servletContext, locale, variables, appctx);
        }
        
        @Override
        public PreFragmentHandler getPreFragmentHandler(String name) {
            return getApplicationContext().getBean(name, PreFragmentHandler.class);
        }
    }
